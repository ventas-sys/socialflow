"""COPIA de sim_b.run_attempt / evaluate con UN cambio adicional, para medir el sesgo de la vela de entrada.
sim_b (conservador, el que usa run.py): si la vela M15 en la que se dispara el stop de entrada toca también el SL
(o ambos lados del rango en OCO), se asume entrada + stop en esa misma vela.
Esta copia (optimista): esa vela nunca ejecuta el SL; la posición sobrevive hasta la vela siguiente. La verdad está
entre ambas; la diferencia acota el peso del sesgo. NO se usa para seleccionar parámetros, solo en el REPORT.md.
"""
            ...
            if d != 0 and not day_stopped and trades_today < max_trades_day and not np.isnan(sl_arr[i]) and sl_arr[i] > 0:
                ...
                if pos:
                    ...
                    if lots <= 0:
                        ...
                        if False:   # CAMBIO (sim_optimista): no se asume stop en la vela de entrada aunque la toque
                            fill_sl = sl_px - pos * slip
