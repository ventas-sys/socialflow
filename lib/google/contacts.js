...
// phone puede venir null: con el formato @lid de WhatsApp el número real queda
// oculto para el bot. En ese caso se agenda igual (nombre + categoría + nota),
// sin inventar un número. chatId (opcional) se anota para referencia.
export async function agendarContacto({ categoria, nombre, cuit, compra, phone, chatId }) {
  const c = cfg();
  ...
  try {
    ...
    if (phone && await yaAgendado(token, phone)) return { skipped: 'ya agendado' };
    ...
    const notas = [
      ...
      phone ? `WhatsApp: ${phone}`
            : '⚠ Número oculto por WhatsApp (chat @lid): abrí el chat en el celular para verlo y completarlo.',
      !phone && chatId ? `Ref chat: ${chatId}` : null,
