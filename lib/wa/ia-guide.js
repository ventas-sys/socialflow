...
import { generarTexto } from '../gemini-texto.js';

// Llama a Gemini para texto. El modelo, el "pensamiento" y los reintentos los
// resuelve lib/gemini-texto.js (ver el comentario de arriba de ese archivo:
// Google dejó de habilitar gemini-2.5-flash a los proyectos nuevos).
// Devuelve la misma forma que traía la respuesta cruda de la API para no tocar
...
async function gemini(prompt) {
  ...
  if (r.ok) return { candidates: [{ content: { parts: [{ text: r.texto }] } }] };
  return { error: { message: String(r.error || 'sin respuesta de Gemini'), code: r.codigo } };
}
