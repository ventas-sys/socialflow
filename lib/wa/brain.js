...
export async function processMessage({ from, text, state, history }) {
  ...
  const iaMeta = { intent: ia.intent, confidence: ia.confidence, needsHuman: ia.needsHuman, contacto: ia.contacto || null };
  ...
  // --- RECLAMO: el cliente ya mandó su usuario/datos → confirmación cálida
  if (ia.intent === 'reclamo_datos') {
    return {
      messages: [{ body: TEMPLATES.reclamoConfirmacion, delaySec: 0 }],
      state: { ...baseState, flagHuman: true },
      matched: { kind: 'ia_reclamo_datos' },
      ia: iaMeta,
      reason: 'ia_reclamo_datos',
    };
  }
