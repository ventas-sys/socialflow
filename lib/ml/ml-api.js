...
// Cuántos multigets salen a la vez (ver getItemsBulk).
const LOTES_EN_PARALELO = 8;

// Trae varias publicaciones de una sola vez (multiget de ML, hasta 20 por llamada).
// Devuelve un Map item_id -> { title, permalink, price, sku, fotos }.
// Se usa en el control de conversión para no pegarle una vez por publicación.
export async function getItemsBulk(token, ids) {
  const out = new Map();
  const lista = [...new Set((ids || []).map(String).filter(Boolean))];
  const lotes = [];
  for (let i = 0; i < lista.length; i += 20) lotes.push(lista.slice(i, i + 20));

  // De a 8 lotes en paralelo. Un catálogo entero son ~105 lotes: de a uno se
  // come 30 s y el reporte de faltantes (que trae DOS catálogos) se pasa de
  // los 60 s de Vercel. Con 8 en paralelo baja a ~4 s y ML no se queja.
  const pedir = (lote) => httpRequest('GET',
    ML + `/items?ids=${lote.join(',')}&attributes=id,title,permalink,price,available_quantity,seller_custom_field,pictures`,
    { 'Authorization': 'Bearer ' + token }).catch(() => null);

  const respuestas = [];
  const fallados = [];
  for (let i = 0; i < lotes.length; i += LOTES_EN_PARALELO) {
    const tanda = lotes.slice(i, i + LOTES_EN_PARALELO);
    const rs = await Promise.all(tanda.map(pedir));
    rs.forEach((r, j) => (r ? respuestas.push(r) : fallados.push(tanda[j])));
  }

  // Un lote que no vuelve son 20 publicaciones que desaparecen del reporte, y
  // en el de faltantes eso se lee como "hay que publicarlo" cuando ya está.
  // Reintento una vez de a uno; si igual no vuelve, mejor cortar que mentir.
  for (const lote of fallados) {
    const r = await pedir(lote);
    if (!r) throw new Error(`ML no respondió el multiget de ${lote.length} publicaciones (${lote[0]}...). Probá de nuevo en un minuto.`);
    respuestas.push(r);
  }

  for (const r of respuestas) {
    for (const entry of (Array.isArray(r?.body) ? r.body : [])) {
      const it = entry?.body;
      if (!it?.id) continue;
      out.set(String(it.id), {
        ...
        fotos: Array.isArray(it.pictures) ? it.pictures.length : null,
      });
    }
  }
  return out;
...
// Multiget con la FICHA TÉCNICA (attributes) para el reporte de medidas.
// Hasta 20 por llamada; los lotes van en paralelo porque el reporte recorre
// todo el catálogo y la función tiene 10s en total (plan Hobby de Vercel).
export async function getItemsFichaBulk(token, ids) {
  const lista = [...new Set((ids || []).map(String).filter(Boolean))];
  const lotes = [];
  for (let i = 0; i < lista.length; i += 20) lotes.push(lista.slice(i, i + 20));
  const out = new Map();
  const respuestas = await Promise.all(lotes.map(lote => httpRequest('GET',
    ML + `/items?ids=${lote.join(',')}&attributes=id,title,permalink,status,seller_custom_field,attributes`,
    { 'Authorization': 'Bearer ' + token }).catch(() => null)));
  for (const r of respuestas) {
    for (const entry of (Array.isArray(r?.body) ? r.body : [])) {
      const it = entry?.body;
      if (it?.id) out.set(String(it.id), it);
    }
  }
  return out;
}

// Una orden puntual (para sacar el comprador, el pack y el producto).
export async function getOrder(token, orderId) {
  return mlGetChecked(token, `/orders/${orderId}`);
}

// Mensajes SIN LEER de una conversación post-venta.
//
// ⚠️ REGLA DE ORO DE LA MENSAJERÍA DE ML: el ÚNICO recurso que marca los
// mensajes como leídos es el GET /messages/packs/{pack}/sellers/{seller}.
// Los demás (este incluido) NO tocan el estado de lectura. Por eso el bot
// puede fijarse si el comprador escribió sin "abrirle" la conversación al
// equipo: lo que estaba sin leer, sigue sin leer.
export async function getUnreadMessages(token, packId, sellerId) {
  return mlGetChecked(token, `/messages/unread/packs/${packId}/sellers/${sellerId}?tag=post_sale`);
}

// Lee la conversación SIN marcarla como leída.
// El `mark_as_read=false` va fijo a propósito: si algún día alguien necesita
// leer los mensajes, que no le deje la conversación marcada como atendida al
// equipo sin que nadie la haya visto.
export async function getPackMessages(token, packId, sellerId) {
  return mlGetChecked(token, `/messages/packs/${packId}/sellers/${sellerId}?tag=post_sale&mark_as_read=false`);
}

// Manda un mensaje al comprador por la mensajería POST-VENTA de ML.
// Es el único canal permitido para escribirle a un comprador; ML modera el
// contenido y bloquea links externos y datos de contacto.
// Mandar un mensaje (POST) NO marca como leída la conversación: el estado de
// lectura de lo que escribió el comprador queda intacto.
export async function sendPostSaleMessage(token, { packId, sellerId, buyerId, text }) {
  const r = await httpRequest('POST',
    ML + `/messages/packs/${packId}/sellers/${sellerId}?tag=post_sale`,
    { 'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json', 'x-format-new': 'true' },
    { from: { user_id: String(sellerId) }, to: { user_id: String(buyerId) }, text });
  const ok = r.status >= 200 && r.status < 300;
  return { ok, status: r.status, body: r.body, error: ok ? null : (r.body?.message || r.body?.error || ('HTTP ' + r.status)) };
}
...
export async function searchMyItems(token, userId, opts = {}) {
  const { q, status, limit = 20, offset = 0, extra } = opts;
  // `extra` va crudo en la query: lo usa el scan (search_type=scan&scroll_id=...)
  // del reporte de medidas, que es la única forma de pasar de 1000 resultados.
  let path = `/users/${userId}/items/search?limit=${limit}`;
  path += extra ? `&${extra}` : `&offset=${offset}`;
...
// Órdenes cuyo estado CAMBIÓ en una ventana de tiempo.
//
// Para el barrido de entregas: cuando un envío pasa a "entregado", la orden se
// actualiza. Filtrando por date_last_updated traemos solo las que se movieron
// hace poco, en vez de recorrer miles de órdenes viejas en cada pasada.
//
// Viene ordenado por fecha descendente y con tope, así una pasada nunca se
// pasa del tiempo que tiene la función.
export async function getOrdersActualizadas(token, sellerId, desde, hasta, limite = 60) {
  const orders = [];
  let offset = 0;
  while (orders.length < limite) {
    const path = `/orders/search?seller=${sellerId}`
      + `&order.date_last_updated.from=${encodeURIComponent(desde)}`
      + `&order.date_last_updated.to=${encodeURIComponent(hasta)}`
...
// La API de publicidad usa el mismo token pero pide el header Api-Version.
// GET crudo que devuelve status + body para poder sondear qué acceso tenemos.
export async function mlAdsGet(token, path, apiVersion) {
  const r = await httpRequest('GET', ML + path, {
    'Authorization': 'Bearer ' + token,
    // La doc de Mercado Ads lo escribe en minúsculas; lo mandamos así.
