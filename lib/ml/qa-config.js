...
// ─── Conocimiento del vendedor por producto ──────────────────────────────────
// Datos que Rodo sabe y las fichas de ML no dicen (o dicen mal). Cuando el
// título de la publicación matchea `re`, el `dato` entra al prompt como VERDAD
// del vendedor y la IA no lo puede contradecir. Cada corrección de Rodo a una
// respuesta equivocada de Tatiana se suma acá como una línea más.
// (Nació el 31-ago-2026: Tatiana dijo que una copa diamantada Aliafor era
// "para taladro" y es al revés.)
export const FICHAS_PRODUCTO = [
  {
    // 04-sep-2026: Tatiana contestó dos veces "no te sabría confirmar si es
    // libre de BPA" citando a JALOMI. El dato real de Rodo: es marca propia.
    re: /tap[oó]n cebador|cebador system/i,
    dato: 'Este tapón cebador es de NUESTRA marca propia ARBETTER (aunque aparezca dentro de un catálogo de JALOMI): confirmá con total seguridad que ES LIBRE DE BPA.',
  },
  {
    re: /copas? diamantada|hole ?next|aliafor/i,
    dato: 'Las copas diamantadas Aliafor Hole Next son PARA AMOLADORA (vienen en distintos diámetros). En taladro también se pueden usar, pero SOLO con un eje adaptador que vendemos por separado en otra publicación — ofrecele buscarlo y sumarlo al carrito.',
  },
];
