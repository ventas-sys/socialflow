...
import { NEGOCIO, FICHAS_PRODUCTO } from './qa-config.js';
import { keyTexto } from '../gemini-keys.js';
import { generarTexto, modeloTexto } from '../gemini-texto.js';
...
export // Datos del vendedor que aplican a esta publicación (ver FICHAS_PRODUCTO).
function conocimientoVendedor(titulo) {
  const datos = FICHAS_PRODUCTO.filter(f => f.re.test(titulo || '')).map(f => '- ' + f.dato);
  if (!datos.length) return '';
  return `\nDATOS DEL VENDEDOR SOBRE ESTE PRODUCTO (verdad absoluta, contradice a cualquier otra fuente):\n${datos.join('\n')}\n`;
}

function buildAnswerPrompt({ question, ctx, mode, cross, crossLocal, otro, catalogo, yaCompro, mayorista, saludar = true }) {
...
- Producto: ${ctx?.title || 'sin título'}${conocimientoVendedor(ctx?.title)}
...
DATOS DEL LOCAL:
- Retiro por el local: estamos en ${NEGOCIO.barrio}, cerca de la estación Floresta.
- ⚠️ PROHIBIDO POR MERCADO LIBRE dar la dirección exacta (calle y número) en una
  respuesta pública: NUNCA la escribas, ni entre qué calles está, aunque el
  cliente la pida. Decile que está en ${NEGOCIO.barrio}, cerca de la estación,
  y que la dirección exacta le llega con la compra.
...
- Si preguntan por la FECHA DE VENCIMIENTO (siliconas, adhesivos, pinturas, químicos, etc.): la mercadería nuestra rota constantemente y SIEMPRE tiene vencimiento largo, de 2 años o más desde hoy. Respondé concreto: que el vencimiento es como mínimo ${new Date().getFullYear() + 2}. NO inventes una fecha impresa exacta ni digas que no sabés.
...
export async function probarIA() {
  ...
  try {
    const r = await generarTexto({ prompt: 'Respondé solo: ok', minOutputTokens: 512 });
    if (r.ok) return { ok: true, modelo: r.modelo };
    return { ok: false, codigo: r.codigo, modelo: r.modelo, error: String(r.error || '').slice(0, 250) };
  } catch (e) {
...
export async function generateAnswer(args) {
  const key = keyTexto();
  if (!key) throw new Error('GEMINI_API_KEY no configurada (ni GEMINI_API_KEY_TEXTO)');
  const prompt = buildAnswerPrompt(args);
  // El pensamiento del modelo y el presupuesto de salida los maneja
  // lib/gemini-texto.js: los tokens que "piensa" Gemini 3 se descuentan de
  // maxOutputTokens, así que un presupuesto chico devolvía respuestas vacías.
  const r = await generarTexto({
    prompt,
    generationConfig: { temperature: 0.6, responseMimeType: 'text/plain' },
  });
  let text = (r.texto || '').trim();
  if (!text) throw new Error('IA sin respuesta: ' + String(r.error || '').slice(0, 200));
  // Red de seguridad (29-ago-2026): ML prohíbe la dirección exacta en respuestas
  // públicas y Tatiana la estaba dando. Aunque el prompt ya lo prohíbe, si la IA
  // igual la escribe, acá se reemplaza por la referencia permitida.
  text = text
    // "entre (las calles) Sanabria y Gualeguaychú" — nombres con mayúscula, así
    // no se come frases comunes como "entre lunes y viernes".
    .replace(/,?\s*entre (las calles )?[A-ZÁÉÍÓÚÑ][a-záéíóúñ]+ y [A-ZÁÉÍÓÚÑ][a-záéíóúñ]+/g, '')
    .replace(/\bBacacay\s*\d*[.,]?\s*/gi, '')
    .replace(/\b(Sanabria|Gualeguaych[uú])\b[.,]?\s*/gi, '')
    .replace(/\ben\s*,\s*/gi, 'en ')
    .replace(/\s+([.,])/g, '$1')
    .replace(/[ \t]{2,}/g, ' ');
